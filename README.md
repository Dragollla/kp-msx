...
По адресу `/subtitleEditor` есть утилита для синхронизации субтитров. Client ID есть либо в логе, либо в Кинопабе в устройствах. Content ID содержится в ссылке на видео. Утилита сделана с помощью Claude, поэтому за качество исполнения я не отвечаю, но оно работает.